                        ...
                        if(FD_ISSET(STDIN, &rfds)) {
                                ...
                                if(strlen(input)>0) { /* this is to avoid cases where user clicks on ENTER with nothing written */
                                        ...
                                        if(strcmp(header, "join")==0) {
                                                ...
                                                if(!join_flag)
                                                        ...
                                                        if(!contact_flag) {
                                                                ...
                                                                if(sscanf(parametros, "%s %s", header, file)==2) {
                                                                        reader = strtok(header, ".");             /* get contact name, surname and keyfile */
                                                                        strcpy(contactname, reader);
                                                                        reader = strtok(NULL, "\0");
                                                                        strcpy(contactsurname, reader);

                                                                        if(strcmp(contactname, name)==0 && strcmp(contactsurname, surname)==0) { /* case in wich use connects to himself*/
                                                                                printf("You can't connect to yourself!\n");
                                                                        }else{
                                                                                if(!QRY(parametros, name_socket, name_server, contactip, contactport)) /*check if user is on the network */
                                                                                        printf("Could not find %s %s on the network.\n", contactname, contactsurname);
                                                                                else{
                                                                                        printf("Client registered under %s:%s.\n", contactip, contactport); /* get client connect info */
                                                                                        sprintf(fileadd,"keyfile/%s.txt", file); /* open keyfile */
                                                                                        fp = fopen (fileadd, "r");
                                                                                        if(fp==NULL) {
                                                                                                printf("Error opening keyfile. Please check your input and try again.\n");
                                                                                        }else{  /* create a new client socket for the chat session */
                                                                                                ...
                                                                                                if(write(contact_socket, buffer, strlen(buffer))==-1) {
                                                                                                        ...
                                                                                                        exit(1); /* if client is already on a session, write will give an error, because user accepts and closes the connection */
                                                                                                }else{
                                                                                                        /* From now on, the code is related to the authentication protocol */

                                                                                                        /*read the byte sent from the other user */
                                                                                                        ...
                                                                                                        /* search the corresponding byte on the keyfile */
                                                                                                        ...
                                                                                                        /* send the byte from the keyfile to the other user */
                                                                                                        if(write(contact_socket, buffer, 6)==-1) {
                                                                                                                printf("Error on writing.\n");
                                                                                                                exit(1);
                                                                                                        }

                                                                                                        /*now, if we have the same keyfile, this user is already authenticated by the other user. its time to send a byte so that we can authenticate him now */

                                                                                                        srand(time(NULL));
                                                                                                        auth_sent = rand() % 256; /* send a random byte */
                                                                                                        ...
                                                                                                        len=read(contact_socket, buffer, 6); /* get byte sent by the other user */
                                                                                                        if(len==-1) {
                                                                                                        ...
                                                                                                        if(auth_file != auth_recv) {   /* compare the two bytes, see if they are equal*/
                                                ...
                                                if(contact_flag) {
                                                        ...
                                                        if(len!=0) {
                                                                parametros[len]='\n';
                                                                /* since we need to be able to write inifinite messages and the write call may not write all the message once, we need to be sure that it sends everything.
                                                                for this porpuse we use a while and enf it when we are sure everything was written */
                                                                while(nleft>0) {
                                ...
                                if(FD_ISSET(me_socket, &rfds)) {
                                        if(!contact_flag) {
                                              ...
                                              /* the next code is similar to the "connect" if*/

                                                k=0;
                                                addrlen = sizeof(me_server);
                                                if((contact_socket=accept(me_socket,(struct sockaddr *)&me_server,  &addrlen))==-1) { /* we accept the connection*/
                                                        printf("Error on accepting\n");
                                                        exit(1);
                                                }
                                                printf("Someone just connected to you... trying to authenticate.\n");

                                                len=read(contact_socket, buffer, sizeof(buffer)); /* reads the name and surname of the other user */
                                                if(len==-1) {
                                                        printf("Error on reading\n");
                                                        exit(1);
                                                } if(len==0) {
                                                ...
                                                srand(time(NULL)); /*sends the challenge to the person who connected */
                                                auth_sent = rand() % 256;
                                                c = (unsigned char)auth_sent;
                                                sprintf(buffer, "AUTH %c", c);
                                                if(write(contact_socket, buffer, 6)==-1) {
                                                ...
                                                len=read(contact_socket, buffer, 6); /* reads the challenge response */
                                                if(len==-1) {
                                                        printf("Error on reading\n");
                                                        exit(1);
                                                } if(len==0) {
                                                ...
                                                sprintf(fileadd,"keyfile/%s%s.txt", contactname, contactsurname); /* opens the respective keyfile to the connection */
                                                fp = fopen (fileadd, "r");
                                                ...
                                                if(fp==NULL) {
                                                        ...
                                                        while ((fgets(byte, sizeof(byte), fp) != NULL )&&(k<line)) /* gets the answer to the challenge from keyfile */
                                                                k++;

                                                        auth_file = atoi(byte);

                                                        if(auth_file != auth_recv) { /*compares the two bytes to fin if it is correct*/
                                                                printf("Could not authenticate %s %s, closing connection...\n",contactname, contactsurname);
                                                                close(contact_socket);
                                                        }else{

                                                                /* now is ther other user turn to authenticate*/

                                                                len=read(contact_socket, buffer, 6); /* we receive the challenge*/
                                                                if(len==-1) {
                                                                        printf("Error on reading\n");
                                                                        exit(1);
                                                                } if(len==0) {
                                                                ...
                                                                k=0;
                                                                fclose(fp); /* find the answer in our file */
                                                                fp = fopen (fileadd, "r");
                                                                while ((fgets(byte, sizeof(byte), fp) != NULL )&&(k<line)) {
                                                                        k++;
                                                                }
                                                                ...
                                                                sprintf(buffer, "AUTH %c", c);  /* send the answer to the other user */
                                                                if(write(contact_socket, buffer, 6)==-1) {
                                                                        printf("Error on writing\n");
                                                                        exit(1);
                                                                }
                                        ...
                                        }else{ /* to solve the problem where a third person tries to connect when user is already in session
                                                  we accept and close the socket*/
                                ...
                                if(FD_ISSET(contact_socket, &rfds)) {
                                  ...
                                  /* this code executes when we are connected to a user and receive a message */




                                        char *str = (char*)malloc(sizeof(char));/* allocate an initial byte */
                                        /* since we need to be able to receive infinite messages, the trick here is to allocate and reallocate memory for each byte we receive */
                                        int msgFlag=1;
                                        int i = 0;
                                        int j = 2;
                                        char msgByte;
                                        int msgClose = 0;
                                        while (msgFlag) { /* while message didnt end, in other others, we didnt read a '\n'*/
                                                len=read(contact_socket, &msgByte, sizeof(char)); /* read a byte */
                                                if(len==-1) {
                                                        printf("Error on reading\n");
                                                        exit(1);
                                                } if(len==0) {
                                                        ...
                                                        if(msgByte=='\n') {
                                                                ...
                                                                str = (char*)realloc(str,j*sizeof(char)); /* allocate memory for another byte */
                                                                str[i] = msgByte; /* save byte read */
                                                                i++;
                                                                j++;
                                                        }
                                                }
                                        }
                                        if(msgClose) {   /* terminate message with NULL char */
                                                str[i]='\0';
